...
func (parser *Parser) ParseApi(searchDir string, mainApiFile string) {
	...
	//解析入口文件api
	//parser.ParseGeneralApiInfo(filepath.Join(searchDir, mainApiFile))
		...
		for _, comment := range fileTree.Comments {
			for _, commentLine := range strings.Split(comment.Text(), "\n") {
				...
				case "@host":
					...
					parser.swagger.Host = "localhost" + config.Config.Addr
				case "@basepath":
					parser.swagger.BasePath = strings.TrimSpace(commentLine[len(attribute):])
				case "@schemes":
					parser.swagger.Schemes = GetSchemes(commentLine)
				case "@tags":
					parser.swagger.Tags = append(parser.swagger.Tags, GetTags(commentLine))
				}
			}
		}
	}
...
// GetTags parses swagger tags for gived commentLine
...
func (parser *Parser) getAllGoFileInfo(searchDir string) {
	//TODO 扫包规则需要更改
	apiPackages := strings.Split(searchDir, ",")
	apiPackages = append(apiPackages, UtilResponse)
	for _, api := range apiPackages {
		filepath.Walk(filepath.Join(PROPATH, SRCPKG, api), func(path string, f os.FileInfo, err error) error {
		...
		//ctrs := strings.Split(config.Config.Swagger.Controller, ",")
		//for _, ctr := range ctrs {
		//	filepath.Walk(filepath.Join(PROPATH, SRCPKG, api, ctr), func(path string, f os.FileInfo, err error) error {
		//		//exclude vendor folder
		//		if ext := filepath.Ext(path); ext == ".go" && !strings.Contains(string(os.PathSeparator)+path, string(os.PathSeparator)+"vendor"+string(os.PathSeparator)) {
		//			astFile, err := goparser.ParseFile(token.NewFileSet(), path, nil, goparser.ParseComments)
		//			if err != nil {
