...
func (operation *Operation) ParseParamComment(commentLine string) error {
	...
	if matches := re.FindStringSubmatch(paramString); len(matches) != 6 {
		...
		case "body", "formData":
			...
			if isSchemaType {
				param = createParameter(paramType, description, name, schemaType, required) // TODO: if Parameter types can be objects, but also primitives and arrays
			} else {
...
func (operation *Operation) ParseAcceptComment(commentLine string) error {
	...
	for _, a := range accepts {
		...
		case "www-form", "application/x-www-form-urlencoded":
...
func createParameter(paramType, description, paramName, schemaType string, required bool) spec.Parameter {
	...
	if paramType == "body" {
		...
		schemaType = changeTypeToSwagger(schemaType)
		parameter := spec.Parameter{
			ParamProps: paramProps,
			SimpleSchema: spec.SimpleSchema{
				Type: schemaType,
...
//change type to swagger
func changeTypeToSwagger(schemaType string) (schType string) {
	var shouldTransInt = []string{"int", "int64", "uint64", "int32", "uint32", "int8", "uint8"}
	var shouldTransFloat = []string{"float64", "float32", "float8", "float"}
